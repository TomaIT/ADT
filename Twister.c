...
static int   state[N+1];     // state vector + 1 extra to not violate ANSI C
static int   *next;          // next random value is computed from here
...
void seedMT(int seed){
    ...
    register int x = (seed | 1U) & 0xFFFFFFFFU, *s = state;
    register int    j;
...
int reloadMT(void){
    register int *p0=state, *p2=state+2, *pM=state+M, s0, s1;
    register int    j;
